...
# Version: 0.4.2
# Description: A Rofi-based Network Manager for i3/bspwm (and others).
# --- Dependencies Check ---
...
ROFI_THEME_NAME="${ROFI_THEME_NAME:-hypr-net}"
...
if [[ -z "${ROFI_NETWORK_MANAGER_THEME:-}" ]]; then
    for _dir in "${XDG_CONFIG_HOME:-$HOME/.config}/rofi/themes" \
               "${XDG_CONFIG_HOME:-$HOME/.config}/rofi/network-manager" \
               "${XDG_CONFIG_HOME:-$HOME/.config}/hypr-net/themes" \
...
if [[ -f "${XDG_CONFIG_HOME:-$HOME/.config}/rofi/network-manager/hypr-net.conf" ]]; then
    source "${XDG_CONFIG_HOME:-$HOME/.config}/rofi/network-manager/hypr-net.conf"
fi
...
send_notification() {
    ...
    if is_notification_service_running && command -v notify-send &> /dev/null; then
        ...
        notify-send -u "$urgency" -a "hypr-net" -i "network-wireless" "$summary" "$body"
    else
...
SCAN_CACHE_FILE="/tmp/hypr-net-scan-cache"
SCAN_CACHE_MAX_AGE=15

perform_wifi_scan() {
    local force_rescan="${1:-no}"

    if [[ "$force_rescan" != "yes" && -f "$SCAN_CACHE_FILE" ]]; then
        local cache_age=$(($(date +%s) - $(stat -c %Y "$SCAN_CACHE_FILE" 2>/dev/null)))
        if [[ $cache_age -lt $SCAN_CACHE_MAX_AGE ]]; then
            cat "$SCAN_CACHE_FILE"
            return
        fi
    fi
    ...
    ')
    echo "$scan_output" > "$SCAN_CACHE_FILE"
    kill_loading_notification
    echo "$scan_output"
}

menu_available_wifi_networks() {
    local wifi_list options chosen force_scan="no"
    while true; do
        wifi_list=$(perform_wifi_scan "$force_scan")
        force_scan="no"
...
menu_wifi() {
	...
	while true; do
		connection_state=$(nmcli --colors no --get-values WIFI general 2>/dev/null)

        local active_uuid=$(nmcli -t -f UUID,DEVICE connection show --active 2>/dev/null | grep ":${interface_to_use}$" | cut -d':' -f1 | head -n 1)

        local active_ssid=""
        local is_ap_mode="no"

        if [ -n "$active_uuid" ]; then
            active_ssid=$(nmcli -g 802-11-wireless.ssid connection show "$active_uuid" 2>/dev/null)
            local mode=$(nmcli -g 802-11-wireless.mode connection show "$active_uuid" 2>/dev/null)
            if [ "$mode" = "ap" ]; then is_ap_mode="yes"; fi
            if [ -z "$active_ssid" ]; then active_ssid=$(nmcli -g connection.id connection show "$active_uuid" 2>/dev/null); fi
        fi
    ...
    while true; do
        ...
        nmcli -t -f UUID,NAME,802-11-wireless.ssid connection show | while IFS=: read -r uuid name ssid; do
            ...
            if [ "$ssid" = "$wifi_ssid" ]; then
                echo "$uuid" > /tmp/hypr_net_uuid_found
                break
            fi
        ...
        if [ -f /tmp/hypr_net_uuid_found ]; then
            existing_uuid=$(cat /tmp/hypr_net_uuid_found)
            rm /tmp/hypr_net_uuid_found
        fi
...
show_qrcode() {
    ...
    local qr_file="/tmp/hypr-net-qr-${ssid}.png"
